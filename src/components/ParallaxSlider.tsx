...
const ParallaxSlider: React.FC = () => {
  ...
  const variants = {
    ...
    center: { x: 0, opacity: 1, scale: 1, transition: { duration: 0.8, ease: [0.25, 0.46, 0.45, 0.94] as const } },
    exit: (dir: number) => ({ x: dir > 0 ? "-30%" : "30%", opacity: 0, scale: 0.95, transition: { duration: 0.6, ease: [0.25, 0.46, 0.45, 0.94] as const } }),
  };
    ...
    <div className="relative w-full h-[85vh] min-h-[600px] overflow-hidden rounded-3xl">
      <AnimatePresence custom={direction} mode="wait">
        <motion.div key={slide.id} custom={direction} variants={variants} initial="enter" animate="center" exit="exit" className="absolute inset-0">
          <motion.div className="absolute inset-0" animate={{ scale: 1.05 }} transition={{ duration: 6, ease: "linear" }}>
          ...
          <div className="absolute inset-0 flex items-center">
            <div className="container mx-auto px-6 lg:px-12">
              <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.6 }} className="max-w-2xl">
                <span className="inline-block btn-pill bg-primary/20 text-primary text-sm mb-4 border border-primary/30">{slide.subtitle}</span>
                <h1 className="text-5xl md:text-7xl font-bold font-display mb-4 leading-tight text-foreground">{slide.title}</h1>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 z-10">
        <button onClick={prev} className="glass rounded-full p-2 text-foreground hover:text-primary transition-colors"><ChevronLeft className="w-5 h-5" /></button>
        <div className="flex gap-2">
          {slides.map((_, i) => (
            <button key={i} onClick={() => goTo(i)}
              className={`h-2 rounded-full transition-all duration-300 ${i === current ? "w-8 bg-primary glow-primary" : "w-2 bg-muted-foreground/40 hover:bg-muted-foreground/60"}`} />
          ))}
        </div>
        <button onClick={next} className="glass rounded-full p-2 text-foreground hover:text-primary transition-colors"><ChevronRight className="w-5 h-5" /></button>
      </div>
    </div>
  );
};
