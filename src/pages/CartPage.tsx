          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-4">
              ...
              <AnimatePresence>
                {cartItems.map((item) => {
                  ...
                  return (
                    <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -100 }} className="glass rounded-3xl p-4 sm:p-5">
                      <div className="flex gap-4">
                        <Link to={`/product/${product.slug}`} className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl overflow-hidden shrink-0">
                          <img src={product.thumbnail || "/placeholder.svg"} alt={product.name} className="w-full h-full object-cover" />
                        </Link>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <Link to={`/product/${product.slug}`} className="font-semibold text-foreground hover:text-primary transition-colors line-clamp-1">{product.name}</Link>
                              {variantLabel && <Badge variant="secondary" className="text-[10px] mt-1">{variantLabel}</Badge>}
                            </div>
                            <span className="font-bold text-foreground whitespace-nowrap">{formatPrice(itemPrice * item.quantity)}</span>
                          </div>
                          <div className="flex items-center gap-2 mt-1.5">
                            <span className="text-sm text-foreground">{formatPrice(itemPrice)} each</span>
                            {product.compare_at_price && (
                              <span className="text-xs text-muted-foreground line-through">{formatPrice(product.compare_at_price)}</span>
                            )}
                          </div>
                          {savings > 0 && <p className="text-xs text-green-500 mt-0.5">You save {formatPrice(savings)}</p>}
                          {maxQty <= 5 && <p className="text-xs text-orange-400 mt-0.5 flex items-center gap-1"><AlertCircle className="w-3 h-3" /> Only {maxQty} left</p>}
                          <div className="flex items-center justify-between mt-3">
                            <div className="flex items-center gap-1 glass rounded-full px-1 py-0.5">
                              ...
                              <button onClick={() => updateQty.mutate({ id: item.id, quantity: Math.min(maxQty, item.quantity + 1) })} disabled={item.quantity >= maxQty} className="p-1.5 rounded-full hover:bg-secondary/50 disabled:opacity-30"><Plus className="w-3 h-3" /></button>
                            </div>
                            <div className="flex gap-1">
                              <button onClick={() => moveToWishlist(product.id, item.id)} className="p-2 rounded-full text-muted-foreground hover:text-primary hover:bg-secondary/50" title="Move to wishlist">
            ...
            <div className="space-y-4">
              <div className="glass-strong rounded-3xl p-6 sticky top-24 space-y-4">
                ...
                <Link to="/checkout" state={{ coupon: appliedCoupon, giftWrap, giftMessage, orderNotes, shippingMethodId: selectedShipping }} className="block">
                  <motion.span whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    className="w-full btn-pill bg-gradient-primary text-primary-foreground font-semibold py-3.5 flex items-center justify-center gap-2 text-base">
                    Proceed to Checkout <ArrowRight className="w-5 h-5" />
                  </motion.span>
                </Link>

                <p className="text-center text-xs text-muted-foreground">🔒 Secure checkout</p>
              </div>
            </div>
          </div>
        )}
