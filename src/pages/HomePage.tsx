...
const HomePage: React.FC = () => {
  ...
  // Flash sale config
  const { data: flashSaleConfig } = useQuery({
    queryKey: ["flash-sale-config"],
    queryFn: async () => {
      const { data, error } = await supabase.from("site_settings").select("value").eq("key", "flash_sale_config").maybeSingle();
      if (error) throw error;
      if (!data?.value) return { enabled: true, title: "Flash Sale Live!", subtitle: "Up to 70% off on selected items. Limited time only.", button_text: "Shop Flash Sale", button_link: "/shop" };
      const val = data.value as any;
  ...
  return (
    <div className="min-h-screen pb-20 lg:pb-0">
      ...
      <main className="container mx-auto px-4 pt-6 space-y-16">
        ...
        {/* Flash Sale Banner - configurable */}
        {flash.enabled !== false && (
          <motion.section initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="glass-strong rounded-3xl p-8 md:p-12 relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-2xl md:text-3xl font-bold font-display text-foreground">{flash.title || "Flash Sale Live!"}</h3>
                  <p className="text-muted-foreground">{flash.subtitle || "Up to 70% off on selected items."}</p>
                </div>
              </div>
              <a href={flash.button_link || "/shop"} className="btn-pill bg-gradient-accent text-accent-foreground font-semibold px-8 py-3 whitespace-nowrap">
                {flash.button_text || "Shop Flash Sale"}
              </a>
            </div>
          </motion.section>
        )}
