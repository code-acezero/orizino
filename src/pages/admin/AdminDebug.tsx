...
const EDGE_FUNCTIONS = [
  "send-push", "get-vapid-key", "ai-chat", "notify-live-support",
  "notify-restock", "fetch-exchange-rates", "get-ice-servers",
  "track-visit", "generate-invoice", "generate-sitemap", "og-image",
  "create-order", "sync-payment-proof", "sync-shipments",
  "courier-secrets", "pathao", "steadfast", "sync-recording-to-drive",
];
...
const AdminDebug: React.FC = () => {
  ...
  const ping = async (name: string) => {
    setPingStatus((s) => ({ ...s, [name]: "pending" }));
    try {
      const { error } = await supabase.functions.invoke(name, { body: { __ping: true } });
      // We treat 4xx/5xx as "fail", any other resolved call as ok
      setPingStatus((s) => ({ ...s, [name]: error ? "fail" : "ok" }));
    } catch {
      setPingStatus((s) => ({ ...s, [name]: "fail" }));
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-6 md:py-8 max-w-5xl space-y-6">
      ...
      <Tabs defaultValue="push">
        <TabsList className="rounded-2xl p-1 grid grid-cols-2 md:grid-cols-4 gap-1">
          <TabsTrigger value="push" className="rounded-xl"><Bell className="w-4 h-4 mr-1.5" />Push</TabsTrigger>
          <TabsTrigger value="calls" className="rounded-xl"><Phone className="w-4 h-4 mr-1.5" />Calls</TabsTrigger>
          <TabsTrigger value="edge" className="rounded-xl"><Server className="w-4 h-4 mr-1.5" />Edge fns</TabsTrigger>
          <TabsTrigger value="realtime" className="rounded-xl"><Radio className="w-4 h-4 mr-1.5" />Realtime</TabsTrigger>
        </TabsList>

        <TabsContent value="push" className="space-y-5 mt-5">
        ...
        <TabsContent value="edge" className="space-y-5 mt-5">
          <Section title="Edge function pings" desc="Sends an empty {__ping:true} body to each function. Errors are normal for functions that require structured input — use the result as an availability check, not a contract test." icon={<Server className="w-5 h-5 text-primary" />}>
                ...
                return (
                  <div key={name} className="flex items-center justify-between gap-2 p-2.5 rounded-xl bg-secondary/30 border border-border/40">
                    ...
                    <div className="flex items-center gap-2">
                      {st === "ok" && <Badge variant="outline" className="text-green-500 border-green-500/30"><Check className="w-3 h-3 mr-1" />ok</Badge>}
                      {st === "fail" && <Badge variant="outline" className="text-destructive border-destructive/30"><X className="w-3 h-3 mr-1" />fail</Badge>}
                      {st === "pending" && <Loader2 className="w-3.5 h-3.5 animate-spin text-muted-foreground" />}
        ...
        <TabsContent value="realtime" className="space-y-5 mt-5">
