          ...
          <Card className="glass">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="relative">
                ...
                <Input
                  value={featCatSearch}
                  onChange={(e) => setFeatCatSearch(e.target.value)}
                  ...
                  .map((cat, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Badge variant={cat.is_featured ? "default" : "outline"} className={`text-[10px] shrink-0 ${cat.is_featured ? "" : "text-muted-foreground"}`}>
                          {cat.is_featured ? "Featured" : "Hidden"}
                        </Badge>
                        <Switch
          ...
          <Card className="glass">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <Input
                  value={featProdSearch}
                  onChange={(e) => setFeatProdSearch(e.target.value)}
                ...
                {localProducts
                  ...
                  .map((prod, idx) => {
                    ...
                    return (
                      <div
                        key={prod.id}
                        {...getFeatProdDragProps(idx)}
                        className={`flex items-center gap-3 p-3 rounded-xl border transition-all cursor-grab active:cursor-grabbing ${
                          isDragging ? "opacity-50 scale-95 border-border" :
                          isOver ? "ring-2 ring-primary/40 border-primary/30 bg-primary/5" :
                          prod.is_featured ? "border-primary/20 bg-primary/5" : "border-border bg-secondary/10"
                        }`}
                      >
                        <GripVertical className="w-4 h-4 text-muted-foreground/40 shrink-0" />
                        <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center text-sm font-bold text-primary shrink-0">
                        ...
                        <Badge variant={prod.is_featured ? "default" : "outline"} className={`text-[10px] shrink-0 ${prod.is_featured ? "" : "text-muted-foreground"}`}>
                          {prod.is_featured ? "Featured" : "Not Featured"}
                        </Badge>
                        <Switch
